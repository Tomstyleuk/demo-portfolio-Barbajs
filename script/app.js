...
btn.forEach((i, index) => {
  i.addEventListener('click', (e) => {
    ...
    // FIRST
    let fullImgRect = i.parentNode.previousElementSibling.getBoundingClientRect();
    let smallImgRect;
    let detailTitle;
    let detailWrapper;
    let innerDetail;
      ...
      detailItems.forEach(d => {
        ...
        if (detailItem === parentItem) {
          ...
          if (main.dataset.state == 'detail') {
            ...
            // if (nextSection) {
            //   let nextTopImg = nextSection.querySelector(".img_container img");
            //   let hoverImgRect;

            //     nextProjectLink.addEventListener('mouseenter', () => {
            //       hoverImg.style.background = `url(${nextTopImg.getAttribute("src")}) no-repeat`;
            //       hoverImg.style.backgroundSize = `400px auto`;
            //       hoverImg.style.backgroundPosition = `bottom center`;
            //       const keyframe = [
            //         { opacity: '0' },
            //         { opacity: '1' },
            //       ];
            ...
            //         iterations: 1
            //       };
            //       hoverImg.animate(keyframe, timing);
            //     })

            //     nextProjectLink.addEventListener('mouseleave', () => {
            //       hoverImg.style.background = ``;
            //     })
            // }// end if (nextSection)
